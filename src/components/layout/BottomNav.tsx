...
const BottomNav = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:w-96 md:mx-auto">
      <nav className="bg-[#14141f]/80 backdrop-blur-md border-t border-[#2d2d3d] p-1 rounded-t-2xl">
            ...
            <li key={item.path} className="relative">
              <Link
                ...
                className={cn(
                  ...
                  item.active ? "text-violet-400" : "text-gray-500"
                )}
              >
                {item.active && (
                  <motion.div
                    layoutId="navIndicator"
                    className="absolute inset-0 bg-[#252536] rounded-xl z-0"
                    initial={false}
                    transition={{
                      type: "spring",
                      stiffness: 400,
